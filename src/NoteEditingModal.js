    ...
    render() {
        ...
        const contentList = {
            ...
            return (
                ...
                <Card type = "inner" loading={this.state.isLoading} hoverable title={ <Popover trigger = "click" content = { <Input onChange = { this.changeTitle.bind(this) } value = { this.state.title } placeholder = "Title input..."/> } >
                <div> {this.state.title} </div> </Popover> } style={{ background: 'white',/* marginTop: '150px'*/ }}
                tabList = {tabList} onTabChange = {(key) => { this.onTabChange(key,'key') } }
                >
